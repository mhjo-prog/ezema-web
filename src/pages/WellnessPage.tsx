...
export default function WellnessPage() {
  ...
  const filtered = posts
    .filter((p) => filter === "전체" || p.wellness_category === filter)
    .filter((p) => {
        ...
        {loading ? (
          ...
          >
            ...
            <p style={{ fontSize: "1rem", fontWeight: 500, color: "#888888" }}>
              {filter === "전체" ? "아직 게시된 글이 없습니다." : `${filter} 콘텐츠를 준비 중입니다.`}
            </p>
            <p style={{ fontSize: "0.875rem", color: "#bbbbbb" }}>매일 새로운 인사이트가 추가됩니다.</p>
          </div>
        ) : (
          <>
            <div
            ...
            >
              {visiblePosts.map((post, i) => (
                ...
                >
                  <WellnessCard post={post} onClick={() => navigate(`/wellness/${post.id}`)} />
                </motion.div>
              ))}
            </div>
            {hasMore && (
