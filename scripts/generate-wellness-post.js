...
// ── Fitt Insider 설정 ────────────────────────────────────────────────
const FITT_KEYWORDS = [
  "wellness", "sleep", "stress", "mental", "connection", "longevity",
  "habit", "mindful", "community", "recovery", "nutrition", "care",
  "well-being", "health",
];
...
// ── Fitt Insider 크롤링 ──────────────────────────────────────────────
async function fetchFittInsider() {
  try {
    // 1. 글 목록 fetch
    const res = await fetch("https://insider.fitt.co/articles/", {
      headers: { "User-Agent": "Mozilla/5.0" },
      signal: AbortSignal.timeout(10000),
    });
    if (!res.ok) {
      console.warn(`Fitt Insider 목록 접근 실패: ${res.status}`);
      return null;
    }
    const html = await res.text();

    // 2. 링크+제목 추출 후 키워드 필터링
    const filtered = [];
    // absolute URL 패턴
    const absRe = /<a[^>]+href="(https:\/\/insider\.fitt\.co\/[^"#?]+)"[^>]*>\s*([^<]{10,200})\s*<\/a>/g;
    // relative URL 패턴
    const relRe = /<a[^>]+href="(\/[^"#?]+)"[^>]*>\s*([^<]{10,200})\s*<\/a>/g;
    const seen = new Set();

    const push = (url, title) => {
      const clean = title.replace(/\s+/g, " ").trim();
      if (!seen.has(url) && FITT_KEYWORDS.some((kw) => clean.toLowerCase().includes(kw))) {
        seen.add(url);
        filtered.push({ url, title: clean });
      }
    };

    let m;
    while ((m = absRe.exec(html)) !== null) push(m[1], m[2]);
    while ((m = relRe.exec(html)) !== null) push(`https://insider.fitt.co${m[1]}`, m[2]);

    if (filtered.length === 0) {
      console.warn("Fitt Insider: 키워드 일치 글 없음");
      return null;
    }

    // 3. 랜덤 1개 선택
    const chosen = filtered[Math.floor(Math.random() * filtered.length)];
    console.log(`Fitt Insider 선택: "${chosen.title}" (${chosen.url})`);

    // 4. 해당 글 본문 800자 추출
    const articleRes = await fetch(chosen.url, {
      headers: { "User-Agent": "Mozilla/5.0" },
      signal: AbortSignal.timeout(10000),
    });
    if (!articleRes.ok) {
      console.warn(`Fitt Insider 글 접근 실패: ${articleRes.status}`);
      return null;
    }
    ...
    const bodyText = bodyMatch
      ? bodyMatch[1].replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim()
      : "";
    const excerpt = bodyText.slice(0, 800);

    if (excerpt.length < 50) {
      console.warn("Fitt Insider: 본문 추출 실패");
      return null;
    }

    return { title: chosen.title, url: chosen.url, excerpt };
  } catch (err) {
    console.warn("Fitt Insider 크롤링 오류 (skip):", err.message);
    return null;
  }
}
...
async function generateWellnessPostContent(category, trends, existingPosts, crossTitles, externalRef) {
  ...
  if (externalRef && externalRef.items.length > 0) {
    if (externalRef.source === "fitt") {
      externalSection =
        `아래 해외 웰니스 트렌드 영문 자료를 참고해서 한국 독자에게 맞게 재해석하고, keepslow의 '이기적이지 않은 건강함' 콘셉트와 연결해서 한국어로 작성해줘:\n` +
        externalRef.items
          .map((n) => `제목: "${n.title}"\n내용: ${n.summary}${n.summary.length >= 800 ? "..." : ""}`)
          .join("\n") +
        "\n";
    } else if (externalRef.source === "dallem") {
      externalSection =
...
async function main() {
  ...
  // 1. 외부 참고 소스 크롤링 — 50% Fitt Insider / 25% 달램 / 25% 네이버+헬스조선
  let externalRef = null;
  const rand = Math.random();
  if (rand < 0.5) {
    console.log("Fitt Insider 크롤링 중... (50% 확률 선택)");
    const fittData = await fetchFittInsider();
    if (fittData) {
      externalRef = { source: "fitt", items: [{ title: fittData.title, summary: fittData.excerpt }] };
      console.log("Fitt Insider 수집 완료");
    }
  } else if (rand < 0.75) {
    console.log("달램 뉴스레터 크롤링 중... (25% 확률 선택)");
    ...
    console.log("웰니스 뉴스 크롤링 중... (25% 확률 선택)");
